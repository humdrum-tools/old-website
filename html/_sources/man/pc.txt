...
D=2, . . . B=11. It outputs one or more Humdrum ``**pc`` spines
containing pitch-class values corresponding to each of the input pitch
tokens. For example, the ``**semits`` token "-13" is transformed to 11
(pc).
...
translated from numeric-only (0,1,2,3 ... 9,10,11) to mixed alphanumeric
(0,1,2,3 ... 9,A,B) or vice versa. (See documentation for ``**pc (2)``.)
...
transformed to \`A' and \`B'. Encodings using mixed alphanumeric values
are often better suited to pattern-matching and searching tasks.
...
input values to the nearest pitch-class. Hence, an input ``**semits``
data token of 5.6 will be rendered in the ``**pc`` output as the value
6. Similarly, the input ``**freq`` value 452.1 will be rendered in the
...
"X15yz" will result in the output "X3yz" -- that is, after translating
15 to pitch-class 3, the "Xyz" signifiers are retained in the output.
The **-x** option is useful for eliminating non-pitch-class-related
...
``**pc`` value \`B' in the token \`BCD' has been maintained in the first
...
-  `**Pertinent description in the Humdrum User
   Guide** <../guide34.html#Pitch-Class_Representation>`__
