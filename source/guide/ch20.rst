...
*section* and *strophe*. We will encounter examples using the `yank <../../man/yank>`_,
`thru <../../man/thru>`_, and `strophe <../../man/strophe>`_ commands.
...
We encountered the `yank <../../man/yank>`_ command earlier in `Chapter 12 <../ch12>`_. Recall that
...
The Humdrum `thru <../../man/thru>`_ command expands *abbreviated format* representations
...
labels are present in a document? This is a task for `grep <http://en.wikipedia.org/wiki/Grep>`_:
...
The strophic passage pertains only to the spine marked `**silbe <../../rep/silbe>`_. The
`**silbe <../../rep/silbe>`_ representation pertains to syllabic text encoding and is a
pre-defined representation in Humdrum. The `**silbe <../../rep/silbe>`_ representation is
discussed in `Chapter 27 <../ch27>`_. Following the strophic passage indicator
...
and the `**silbe <../../rep/silbe>`_ spine. First, we need to create the full length version
...
(In the `**silbe <../../rep/silbe>`_ representation, the vertical bar (|) and the percent sign
...
`thru <../../man/thru>`_command to create a through-composed document from an
...
In `Chapter 37 <../ch37>`_ we will see further examples of how sections and strophes
