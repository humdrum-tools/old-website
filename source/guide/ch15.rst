...
In `Chapter 11 <../ch11>`_ we examined Humdrum tools related to melodic pitch
intervals. This chapter returns to the discussion of intervals by focussing
on two tools pertaining to characterizing harmonic intervals: the `hint <../../man/hint>`_
and `ydelta <../../man/ydelta>`_ commands.
...
The Humdrum `hint <../../man/hint>`_ command calculates harmonic intervals for pitch-
related representations such as `**kern <../../rep/kern>`_, `**pitch <../../rep/pitch>`_,
`` `**solfg```_, and `**Tonh <../../rep/Tonh>`_. As in the case of the **mint** command,
...
always consists of a single `**hint <../../rep/hint>`_ spine. Any number of spines may be
...
`ditto <../../man/ditto>`_ command. The **ditto** command replaces null tokens with the
...
The `ditto <../../man/ditto>`_, command provides two additional options that are worthy of
...
The second `grep <http://en.wikipedia.org/wiki/Grep>`_ counts the total number of intervals by looking for all
...
The `humsed <../../man/humsed>`_ command (described in `Chapter 14 <../ch14>`_) is well suited to this
...
If we now apply `ditto <../../man/ditto>`_ and recalculate the intervals, the resulting
...
translate Example 15.1 to a `**semits <../../rep/semits>`_ representation.
...
using the `ydelta <../../man/ydelta>`_ command. The **ydelta** command is comparable to
`xdelta <../../man/xdelta>`_, however, numerical differences are calculated between
simultaneous numerical values (delta-*y*) rather than between successive
numerical values (delta-*x*).

Like the `hint <../../man/hint>`_ command, **ydelta** always outputs a single spine. The
...
calculate the actual semitone interval distances. The `rid <../../man/rid>`_ command can
...
a suitable sequence of `grep <http://en.wikipedia.org/wiki/Grep>`_ commands. The **ditto** command is used to
...
In this chapter we have seen how to use the `hint <../../man/hint>`_ command to calculate
these various kinds of intervals. We have also seen how `ydelta <../../man/ydelta>`_ can be
