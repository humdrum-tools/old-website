...
are most frequently used in conjunction with the `grep <http://en.wikipedia.org/wiki/Grep>`_ command encountered
in `Chapter 3 <../ch03>`_. `grep <http://en.wikipedia.org/wiki/Grep>`_ is a popular software tool that is available from a
number of manufacturers and sources. Many versions of `grep <http://en.wikipedia.org/wiki/Grep>`_ differ in the
options provided. For example, the version of `grep <http://en.wikipedia.org/wiki/Grep>`_ distributed by the GNU
Software Foundation provides no fewer than 19 options. Some of the most
common options for `grep <http://en.wikipedia.org/wiki/Grep>`_ are identified in Table 10.1.
...
*Common options for the `grep <http://en.wikipedia.org/wiki/Grep>`_ command.*
...
expression passed to `grep <http://en.wikipedia.org/wiki/Grep>`_. The second asterisk means "all files in the
...
meaning of the asterisk. The **-l** option causes `grep <http://en.wikipedia.org/wiki/Grep>`_ to output only the
...
The **-L** option for `grep <http://en.wikipedia.org/wiki/Grep>`_ causes the output to contain a list of files
...
When searching for more complex patterns it may be necessary to use `grep <http://en.wikipedia.org/wiki/Grep>`_
...
A better solution is to pipe the output between two `grep <http://en.wikipedia.org/wiki/Grep>`_ commands. Recall
...
title records (``OTL``) containing the word ``Liebe`` to a second `grep <http://en.wikipedia.org/wiki/Grep>`_,
which passes only those records also containing the word ``Tod``. Since both
`grep <http://en.wikipedia.org/wiki/Grep>`_ commands process the entire input line, it does not matter whether
the word ``Tod`` precedes or follows the word ``Liebe``:

`` grep '!!!OTL.*Liebe' * | grep 'Tod'``

The **-v** option for `grep <http://en.wikipedia.org/wiki/Grep>`_ causes a "reverse" or "negative" output.
...
In conjunction with the `solfa <../../man/solfa>`_ command, `grep <http://en.wikipedia.org/wiki/Grep>`_ can be used to search
...
sixth scale-degree and the raised fourth scale-degree. In `Chapter 4 <../ch04>`_, we
saw that the **solfa** command represents pitches with respect to an encoded
tonic pitch. In the `**solfa <../../rep/solfa>`_ representation, the lowered sixth and
raised fourth degrees will be represented as ``6-`` and ``4+`` respectively.
First we translate the input to the `**solfa <../../rep/solfa>`_ representation, and then we
...
could use two separate `grep <http://en.wikipedia.org/wiki/Grep>`_ commands, eliminating the constraint of
order:
...
causes `grep <http://en.wikipedia.org/wiki/Grep>`_ to output the *filename* rather than the matching record. If
we could pass along these file names to another `grep <http://en.wikipedia.org/wiki/Grep>`_ command, we could
...
`grep <http://en.wikipedia.org/wiki/Grep>`_ command. The **grep -c** command will thus be applied only to those
files already identified by the previous `grep <http://en.wikipedia.org/wiki/Grep>`_ as containing ``Liebe`` in
the title.
...
containing a passage in 6/8 meter. The **-f** option for `grep <http://en.wikipedia.org/wiki/Grep>`_ provides a
...
We would invoke `grep <http://en.wikipedia.org/wiki/Grep>`_ as follows:

`` grep -l -f criteria *``

The **-f** option tells `grep <http://en.wikipedia.org/wiki/Grep>`_ to fetch the file ``criteria`` and use the
...
in `**pitch <../../rep/pitch>`_ data. We could use a file containing the following regular
expressions:
...
expression syntax. Therefore, we should use the `egrep <http://en.wikipedia.org/wiki/Egrep#Variations>`_ rather than the
`grep <http://en.wikipedia.org/wiki/Grep>`_ command with the above expressions. We can count the number of notes
...
The `grep <http://en.wikipedia.org/wiki/Grep>`_ command is usually thought of as a way to find particular
patterns in a file or input stream. However, the various options for `grep <http://en.wikipedia.org/wiki/Grep>`_
(such as -v, -l, and -L) allow `grep <http://en.wikipedia.org/wiki/Grep>`_ to be used for other purposes. It can
...
also seen how the **-f** option for `grep <http://en.wikipedia.org/wiki/Grep>`_ can be used to permit
**OR**-searches where a work needs to conform only to one of a set of
possible criteria.

Although this chapter has focussed principally on the `grep <http://en.wikipedia.org/wiki/Grep>`_ command, the
ensuing chapters will show that regular expressions are used by a wide
variety of commands. In `Chapter 33 <../ch33>`_, many more powerful examples will be
discussed in conjunction with the `find <http://en.wikipedia.org/wiki/Find>`_ command.
