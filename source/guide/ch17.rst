...
Let's begin by discussing the second process. In `Chapter 3 <../ch03>`_ we saw how the
...
occurrences of each unique data token. The `uniq <http://en.wikipedia.org/wiki/Uniq>`_ command described in
`Chapter 3 <../ch03>`_ will eliminate successive duplicate lines. For example, if we
type:
...
`uniq <http://en.wikipedia.org/wiki/Uniq>`_ command is useful for telling us *how many different things* there
...
Recall that the **-c** option for `uniq <http://en.wikipedia.org/wiki/Uniq>`_ will cause a `count' to be
...
As we saw in `Chapter 13 <../ch13>`_, the `rid <../../man/rid>`_ command can be used to eliminate
...
The `cat <http://en.wikipedia.org/wiki/Cat_(Unix)>`_ command can then be used to concatenate the files end-to-end so
...
the UNIX `wc <http://en.wikipedia.org/wiki/Wc_(Unix)>`_ (word count) command. The `wc <http://en.wikipedia.org/wiki/Wc_(Unix)>`_ command provides three
options. With the **-c** option, `wc <http://en.wikipedia.org/wiki/Wc_(Unix)>`_ counts the number of characters in an
input. With the **-w** option, `wc <http://en.wikipedia.org/wiki/Wc_(Unix)>`_ counts the number of words in an input.
A "word" is defined as any sequence of characters delineated by white space,
such as spaces, tabs or new lines. With the **-l** option, `wc <http://en.wikipedia.org/wiki/Wc_(Unix)>`_ counts the
...
be 874. Using the UNIX `awk <http://en.wikipedia.org/wiki/Awk>`_ command will allow us to easily generate the
...
Recall from `Chapter 3 <../ch03>`_ that the `uniq <http://en.wikipedia.org/wiki/Uniq>`_ command provides other options
(besides the **-c** option). The **-d** option causes `uniq <http://en.wikipedia.org/wiki/Uniq>`_ to output
...
considered "the same" by **sort** and `uniq <http://en.wikipedia.org/wiki/Uniq>`_. This means that records such
...
data so only the data of interest is passed to **sort** and `uniq <http://en.wikipedia.org/wiki/Uniq>`_. This
...
`mint <../../man/mint>`_ or `xdelta <../../man/xdelta>`_ commands to generate a suitable representation.
Alternatively, we might want to generate an inventory of scale degrees using
the `deg <../../man/deg>`_ or `solfa <../../man/solfa>`_ commands.

Instead of translating our data, we might wish to edit the data using the
`sed <http://en.wikipedia.org/wiki/Sed>`_ or `humsed <../../man/humsed>`_ stream editors. Suppose we had a file (named
...
Adding the appropriate `humsed <../../man/humsed>`_ command to our pipe:
...
files ``early`` and ``late``. The `humsed <../../man/humsed>`_ command here eliminates all
data with the exception of \*\*kern_ articulation marks. (See `Chapter 6 <../ch06>`_ for details on **kern articulation marks.)
...
> This problem is addressed in `Chapter 36 <../ch36>`_.
