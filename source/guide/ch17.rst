...
.. |Chapter 36|	replace:: Chapter 36
.. _Chapter 36:	../ch36

.. |Chapter 6|	replace:: Chapter 6
.. _Chapter 6:	../ch06
...
combination ``-GLId`` is very common with |rid|_ since only data records are
...
means using the |rid|_ command with one or more options to eliminate
...
**bet data so it won't interfere. This can be done using the Humdrum |extract|_ command.
...
separate spine. In such cases, we will need to use |extract|_ several times
...
is ideally suited for numerical sorting. We've already learned that the**sort**command rearranges input records in alphabetic/numeric order.
...
Recall from |Chapter 3|_ that the `uniq <http://en.wikipedia.org/wiki/Uniq>`_ command provides other options
...
`sed <http://en.wikipedia.org/wiki/Sed>`_ or |humsed|_ stream editors. Suppose we had a file (named
...
input is monophonic |**kern|_ data.
...
data with the exception of |**kern|_\ articulation marks. (See |Chapter 6|_ for details on **kern articulation marks.)
