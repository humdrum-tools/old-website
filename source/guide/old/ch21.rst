...
occur on single lines. As we saw in `Chapter 19 <../ch19>`_, the `context <../../man/context>`_ command
...
`egrep <http://en.wikipedia.org/wiki/Egrep#Variations>`_. For many tasks, the combination of **context** and `grep <http://en.wikipedia.org/wiki/Grep>`_
...
The `patt <../../man/patt>`_ command may be regarded as a two-dimensional version of
`grep <http://en.wikipedia.org/wiki/Grep>`_. Like `grep <http://en.wikipedia.org/wiki/Grep>`_, **patt** searches for lines that match user-
specified regular expressions. However, unlike `grep <http://en.wikipedia.org/wiki/Grep>`_, **patt** can search
...
in `Chapter 4 <../ch04>`_. Recall that the `**Tonh <../../rep/Tonh>`_ system of pitch names allows
...
The `patt <../../man/patt>`_ command will also identify any overlapping patterns. For
...
Recall that the `deg <../../man/deg>`_ command is mode sensitive, whereas the
`solfa <../../man/solfa>`_ command is mode insensitive. That is, in the key of C major,
...
file to the `**deg <../../rep/deg>`_ representation, and then search for raised mediants
...
`ditto <../../man/ditto>`_ command discussed in `Chapter 15 <../ch15>`_; **ditto** can be used to
...
interest, and then translate to the `**hint <../../rep/hint>`_ harmonic-interval
representation. The **-c** option for `hint <../../man/hint>`_ collapses compound
...
notice the use of the **-s** option for `patt <../../man/patt>`_ in order to skip
...
Below is a `**kern <../../rep/kern>`_ encoding of the final two measures along with
corresponding `**hint <../../rep/hint>`_ and `**deg <../../rep/deg>`_ spines. The **hint spine was
generated using `hint <../../man/hint>`_ **-l** in order to generate intervals with
respect to the lowest pitch.

`` !!!COM: Landini, Francesco
`` **kern\*\*kern**kern`**hint <../../rep/hint>`_**deg**deg**deg
`` *clefF4``*clefG2``*clefG2``*``*``*``*
...
So far, we have seen that `patt <../../man/patt>`_ provides two kinds of output. In the
...
harmonizations. Imagine that we already have a `**harm <../../rep/harm>`_ spine containing
...
> etc.
 ...
 In `Chapter 26 <../ch26>`_ we will learn how to collapse several spines into a single
 ...
 **patt**. A user might tag the beginning of motivic or thematic statements,
 ...
 quotations or allusions, stylistic cliches, etc. In `Chapter 35 <../ch35>`_ we will
...
`` *-``
 ...
 The **-m** option for `patt <../../man/patt>`_ invokes a "multi-record matching" mode.
...
--------
 ...
 `pattern <../../man/pattern>`_ command permits an additional regular expression feature that
...
Z?``
 ...
 expression matches one or more successive `**kern <../../rep/kern>`_ data records
...
--------------------
 ...
 As we will see in `Chapter 26 <../ch26>`_, the contents of several spines can be
 ...
 Consider, for example, the following template for the `pattern <../../man/pattern>`_
 command:
...
-------
 ...
 the Humdrum `patt <../../man/patt>`_ and `pattern <../../man/pattern>`_ commands. Each of these tools has
 ...
 `context <../../man/context>`_, `humsed <../../man/humsed>`_, `rid <../../man/rid>`_ and other tools might facilitate
 the searching task. In future chapters will will consider how "similarity"
 tools such as `correl <../../man/correl>`_ and `simil <../../man/simil>`_ can contribute to more
