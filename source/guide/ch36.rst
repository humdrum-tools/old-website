...
```**freq`` <representations/freq.rep.html>`__ and
```**cents`` <representations/cents.rep.html>`__ representations in
`Chapter 4. <guide04.html>`__ Much of this chapter will center on the
```**spect`` <representations/spect.rep.html>`__ representation. Three
tools will be discussed in connection with ``**spect``: the **spect**,
...
``**spect`` scheme. The ```**spect`` <representations/spect.rep.html>`__
...
differs. In the ``**spect`` representation there is no special
...
```**spect`` <representations/spect.rep.html>`__ (acoustic spectral
data) output from a ```**semits`` <representations/semits.rep.html>`__
...
accepts as input any ``**spect`` data, and for each sonority modifies
...
each ``**spect`` data record, **sdiss** produces a single numerical
...
```**kern`` <representations/kern.rep.html>`__ data for violin. The
middle spine provides corresponding ``**spect`` data using the **spect**
command. The right-most spine shows the result of passing the
``**spect`` data through the **sdiss** command.
