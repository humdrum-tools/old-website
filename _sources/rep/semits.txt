...
~~~~~~~~~~~
    ...
    The **\*\*semits** representation is used to represent absolute
    pitch in semitone units with respect to middle C. Middle C is
    designated zero semitones. All other pitches are represented with
    respect to this reference, hence A4 is 9 semits and A3 is -3 semits.
    Fractional values can also be represented, hence 1.5 is midway
    between C#4 and D4.
...
~~~~~~~~~
    ...
    It is recommended that files containing predominantly ``**semits``
    data should be given names with the distinguishing \`.sem'
    extension.
...
~~~~~~~~~~~~~~~~~~
    ...
    inputs:
        ...
        translates **\*\*semits** to **\*\*cents**
        `**freq** <../commands/freq.html>`__
        translates **\*\*semits** to **\*\*freq**
        `**kern** <../commands/kern.html>`__
        translates **\*\*semits** to **\*\*kern**
        `**melac** <../commands/melac.html>`__
        calculate melodic accent values for successive pitches
        `**pc** <../commands/pc.html>`__
        translate **\*\*semits** pitch to **\*\*pc**
        `**pitch** <../commands/pitch.html>`__
        translate **\*\*semits** pitch to **\*\*pitch**
        `**semits** <../commands/semits.html>`__
        change numerical precision of **\*\*semits** values
        `**solfg** <../commands/solfg.html>`__
        translate **\*\*semits** pitch to **\*\*solfg**
        `**tonh** <../commands/tonh.html>`__
        translate **\*\*semits** pitch to **\*\*Tonh**
        `**vox** <../commands/vox.html>`__
        determine active and inactive voices in a Humdrum file

    The following Humdrum commands produce **\*\*semits** data as
    outputs:

        +----+--------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
        |    | `**semits** <../commands/semits.html>`__   | translates **\*\*cents**, **\*\*freq**, **\*\*fret**, **\*\*kern**, **\*\*MIDI**, **\*\*pitch**, **\*\*semits**, **\*\*solfg**, **\*\*specC**, **\*\*Tonh**, to **\*\*semits**   |
        +----+--------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
