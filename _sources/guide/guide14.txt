...
repertory. In this chapter we will introduce two stream editors: **sed** and
`**humsed**`_.
...
The `**humsed**`_ command is simply a Humdrum version of the UNIX **sed**
stream editor. The syntax and operation of **sed** and **humsed** are
virtually identical. However, **humsed** will modify only Humdrum data
records, whereas **sed** will modify any type of record, including Humdrum
...
way is to give the substitution as a command-line argument to **sed** or
**humsed**:
...
*pizzicato* marks are really *spiccato* marks. In the ```**kern```_
...
the letter ``g``. Without any option, the **sed** and **humsed** substitute
...
`` `**MIDI```_ data; the **grep** command ensures that possible barlines are
...
interest. Stream editors like **sed** and **humsed** can be used to
...
Incidentally, neither the **sed** nor the **humsed** commands support
...
comments or interpretations. The **grep** command has no such restriction.
...
either **sed** or `**humsed**`_) to execute from the file. Consider, for
...
We can use the Humdrum `**pattern**`_ command (to be described in `Chapter
21`_), as follows:
...
The **sed** and `**humsed**`_ commands provide stream editors that can
...
instructions. It should be noted that the **sed** and **humsed** commands
provide many more editing facilities than those discussed in this chapter.
Some 25 operations are provided by **sed** and **humsed**. For example,
...
editing using **sed**, refer to the book on **sed** and **awk** written by
