...
`Themefinder <http://www.themefinder.com>`__ is a web-based application
...
When a search is executed, a "results-page" is generated listing all of
...
10 themes presented on each page.
...
Each theme is represented by a single `Humdrum <../guide05.html>`__
file, designated with the ".thm" file extension. A theme file normally
contains a single `\*\*kern <../representations/kern.rep.html>`__ spine
...
lyrics/text (`\*\*silbe <../representations/silbe.rep.html>`__,
`\*\*text <../representations/text.rep.html>`__), harmonic function
(`\*\*harm <../representations/harm.rep.html>`__), or another other
...
Records <../guide.append1.html>`__ that are used for bibliographic,
text, or reference-related searching. Theme files should minimally
contain reference records identifying the
`composer <../guide.append1.html#COM>`__ (!!!COM) and
`title <../guide.append1.html#OTL>`__ (!!!OTL). Additional reference
...
rendering text in italics -- hence the title will appear italicized.
...
`ms <ms.html>`__ command, which in turn makes use of the *Mup* music
...
refer to the documentation for the `ms command <ms.html>`__.
...
info file is shown below:
    ...
    23049834059834
     ...
     @LINKTHEME: [points to numbered files that contains web link to
    this theme]
    ...
    work]
     @LINKCOLLECTION: [points to numbered files that contains web link
    to this collection]
     @END: INFO
...
copies the old director, e.g. /img becomes /img-old
...
> but this file is ultimately copied to something like 122293375723.thm.
...
-  Section III is other sorts of documentation not covered in Sections I
   and II.
...
#. not encode the rest of the theme.
#. place the rest of the theme in global comment records
...
The key signature in the example above, "\*k[]", contains no sharps or
...
#. **movement** -- themes which belong to the same movement.
#. **work** -- themes which belong to the same composition which may
   consist of several movements.
#. **collection** -- themes from compositions which are related in some
   way. such as the same opus number, or same genre.
...
that in the "work" level grouping "file6" is included.
...
#. providing buttons [M], [W], and [C] for each theme that is a member
   ...
   first movement, and [W] which lists all themes in the 5th symphony.
#. Web hyperlinks can be associated with themes on various levels:
...
-- where the notation of the theme is polyphonic.
...
Other conversion tools available:
    ...
    (3) MUSTRAN to kern
     (4) DARMS to kern -- this is a specialized converter which
    converted the Latin Motet incipit collection into Humdrum.
